    ...
    <section className="overflow-hidden rounded-2xl border border-[var(--border-soft)] bg-[var(--surface)]">
      ...
      {loading ? <p className="p-10 text-center text-sm text-[var(--text-muted)]">Cargando usuarios...</p> : users.map((user) => <div key={user.id} className="border-b border-[var(--border-soft)] last:border-0">
        <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
          ...
          <input aria-label={`Área de ${user.email}`} defaultValue={user.area ?? ''} onBlur={(event) => void changeArea(user, event.target.value)} placeholder="Área (opcional)" disabled={saving === user.id} className="rounded-lg border border-[var(--border)] bg-[var(--surface-elevated)] px-3 py-2 text-sm placeholder-[var(--text-muted)] disabled:opacity-60"/>
          <select aria-label={`Rol de ${user.email}`} value={user.role} disabled={saving === user.id} onChange={(event) => void changeRole(user, event.target.value as UserRole)} className="rounded-lg border border-[var(--border)] bg-[var(--surface-elevated)] px-3 py-2 text-sm disabled:opacity-60"><option value="admin">Administrador</option><option value="analyst">Analista</option><option value="worker">Trabajador</option></select>
          {user.role !== 'admin' && <button type="button" onClick={() => setExpanded(expanded === user.id ? null : user.id)} className="flex items-center gap-2 rounded-xl border border-[var(--border)] px-3 py-2 text-sm hover:bg-[var(--surface-elevated)]">{expanded === user.id ? <ChevronUp size={17}/> : <ChevronDown size={17}/>}Permisos</button>}
        </div>
        {expanded === user.id && user.role !== 'admin' && <div className="overflow-x-auto border-t border-[var(--border-soft)] bg-[var(--surface-elevated)] p-5"><table className="w-full min-w-max">
          <thead className="sticky left-0"><tr>{['Módulo', ...actions].map((label) => <th key={label} className="border-b border-[var(--border)] pb-2 text-left text-xs font-semibold uppercase text-[var(--text-muted)]">{label}</th>)}</tr></thead>
          <tbody>{modules.map((module) => <tr key={module}><td className="border-b border-[var(--border)] py-3 pr-4 text-sm font-medium text-[var(--text-primary)]">{moduleLabels[module]}</td>{actions.map((action) => <td key={action} className="border-b border-[var(--border)] py-3 px-3 text-center"><button type="button" disabled={saving === `${user.id}-${module}`} onClick={() => void toggle(user.id, module, action)} className={`inline-flex h-6 w-6 items-center justify-center rounded-lg text-sm font-medium transition-colors disabled:opacity-60 ${permissions[user.id]?.[module]?.[action] ? 'bg-[var(--accent)] text-white' : 'bg-[var(--border)] text-[var(--text-muted)]'}`}>✓</button></td>)}</tr>)}</tbody>
        </table></div>}
      </div>)}
    </section>
  </div>
