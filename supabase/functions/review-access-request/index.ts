...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if(decision==='rejected'){
      ...
      await sendEmail(accessRequest.email,'Solicitud de acceso a Kargia',`<p>Hola ${escapeHtml(accessRequest.first_name)},</p><p>Tu solicitud de acceso fue rechazada.</p>`).catch(()=>console.error('No se pudo enviar el correo de rechazo.')); return json({ok:true})
    }
